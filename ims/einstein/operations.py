...
class BMI:
    ...
    def provision(self, node_name, img_name, network, channel, nic):
        try:
            ...
            self.fs.clone(str(img_id).encode('utf-8'),
    ...
    def create_snapshot(self, node_name, snap_name):
        try:
            ...
            self.fs.clone(ceph_img_name, constants.DEFAULT_SNAPSHOT_NAME,
                          snap_img_id)
            self.fs.flatten(snap_img_id)
            self.fs.snap_image(snap_img_id, constants.DEFAULT_SNAPSHOT_NAME)
            self.fs.snap_protect(snap_img_id, constants.DEFAULT_SNAPSHOT_NAME)

            self.fs.snap_unprotect(ceph_img_name,
                                   constants.DEFAULT_SNAPSHOT_NAME)
            self.fs.remove_snapshots(ceph_img_name,
                                     constants.DEFAULT_SNAPSHOT_NAME)
            return BMI.__return_success(True)
    ...
    def list_snapshots(self):
        try:
            ...
            return BMI.__return_success(
                self.db.image.fetch_snapshots_from_project(self.project))
    ...
    def remove_image(self, img_name):
        try:
            ...
            self.db.image.delete_with_name_from_project(
                self.db.image.fetch_name_with_id(ceph_img_name[3:]),
                self.project)
            return BMI.__return_success(True)
        except (HaaSException, DBException, FileSystemException) as e:
            self.logger.exception('')
